...
if (GOOGLE_CLIENT_ID && GOOGLE_CLIENT_SECRET) {
    ...
    new GoogleStrategy(
      ...
      async (_accessToken: string, _refreshToken: string, profile: GoogleProfile, done: VerifyCallback) => {
        try {
          ...
          console.error('[Google OAuth Verify Error]:', err?.message || err, err?.stack);
          done(err as Error);
        }
      }
    )
  ...
  console.warn('⚠️ Google OAuth credentials missing; Google sign-in disabled.');
}
...
if (GITHUB_CLIENT_ID && GITHUB_CLIENT_SECRET) {
  ...
  console.warn('⚠️ GitHub OAuth credentials missing; GitHub sign-in disabled.');
}
